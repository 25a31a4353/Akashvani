async function testLineament() {
  const url = new URL('https://bhuvan-vec2.nrsc.gov.in/bhuvan/wms');
  url.searchParams.set('SERVICE', 'WMS');
  url.searchParams.set('VERSION', '1.1.1');
  url.searchParams.set('REQUEST', 'GetFeatureInfo');
  url.searchParams.set('LAYERS', 'lineament:AS_LN50K_0506');
  url.searchParams.set('QUERY_LAYERS', 'lineament:AS_LN50K_0506');
  ...
  console.log('Lineament features:', json.features?.length);
  if (json.features?.[0]) {
    console.log('Feature ID:', json.features[0].id);
    console.log('Properties:', JSON.stringify(json.features[0].properties, null, 2));
