...
const initialLayers = { population: true, vulnerable: false, flood: true, landslide: false, rainfall: true, temperature: false, aqi: false, infrastructure: true, boundaries: true, nationalStates: true, terrain: true, geology: false, liveWeather: true, wind: true, earthquakeSensitivity: true, landslideSensitivity: true, floodSensitivity: true, combinedSensitivity: false, redZones: true, facilities: true };

const layerGroups = [
  { label: "India overview", items: nationwideLayerControls },
  { label: "Exposure", items: [["population", "Population density"], ["vulnerable", "Vulnerable population"]] },
  { label: "Hazard", items: [["flood", "Flood & erosion zones"], ["landslide", "Landslide susceptibility"]] },
  { label: "Environment", items: [["rainfall", "Extreme rainfall"], ["temperature", "Temperature anomaly"], ["aqi", "Air quality index"]] },
  { label: "Reference", items: [["infrastructure", "Critical infrastructure"], ["boundaries", "Administrative boundary"]] },
] as const;
        ...
        <div className="mx-auto max-w-[1660px]">{locationNotice && <div role="status" className="mb-3 flex items-center gap-2 rounded-xl border border-[#ead6a3] bg-[#fff8e7] px-3 py-2 text-[11px] font-medium text-[#795c16]"><CircleAlert className="h-4 w-4 shrink-0" />{locationNotice}</div>}
          ...
          {(["dataset", "replay", "whatif", "cases"] as Workspace[]).includes(workspace) ? <HistoricalLab initialView={workspace === "dataset" ? "dataset" : workspace === "replay" ? "replay" : workspace === "whatif" ? "whatif" : "cases"} /> : <><section data-diva-workspace={workspace} className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_338px]">
            <div className="relative min-w-0 overflow-hidden rounded-2xl border border-[#dbe6e9] bg-white shadow-[0_18px_40px_rgba(22,55,70,0.09)]">
              ...
              <div className="absolute left-3 top-16 z-20 flex flex-col gap-2"><Button variant="outline" size="sm" onClick={() => setIsLayerOpen(!isLayerOpen)} className="h-9 rounded-xl border-white/90 bg-white/95 px-3 text-xs font-semibold text-[#2e5367] shadow-md backdrop-blur"><Layers3 className="mr-1.5 h-3.5 w-3.5 text-[#1e7890]" /> Layers <ChevronDown className={cn("ml-1 h-3.5 w-3.5 transition-transform", isLayerOpen && "rotate-180")} /></Button>{isLayerOpen && <div className="max-h-[410px] w-[244px] overflow-y-auto rounded-xl border border-white/90 bg-white/95 p-3 shadow-xl backdrop-blur"><div className="flex items-center justify-between"><p className="text-xs font-bold text-[#284b60]">Map layers</p><span className="rounded bg-[#e9f5f7] px-1.5 py-0.5 text-[9px] font-bold uppercase text-[#1d7084]">{indiaLocation.id === "india-overview" ? "India overview" : "India context"}</span></div>{layerGroups.map(group => <div key={group.label} className="mt-3 border-t border-[#e7edef] pt-2.5"><p className="mb-1.5 text-[9px] font-bold uppercase tracking-[0.12em] text-[#87959d]">{group.label}</p>{group.label === "India overview" ? <IndiaOverviewLayerControls layers={layers} onChange={(id, checked) => setLayers(current => ({ ...current, [id]: checked }))} /> : group.items.map(([id, label]) => <label key={id} className="flex cursor-pointer items-center justify-between gap-3 py-1.5 text-xs text-[#4c6472]"><span>{label}</span><Switch checked={layers[id]} onCheckedChange={checked => setLayers(current => ({ ...current, [id]: checked }))} aria-label={`Toggle ${label}`} /></label>)}</div>)}<div className="mt-3 border-t border-[#e7edef] pt-3"><p className="mb-1.5 text-[9px] font-bold uppercase tracking-[0.12em] text-[#87959d]">Base style</p><div className="grid grid-cols-2 gap-1 rounded-lg bg-[#edf4f5] p-1"><button onClick={() => setBaseStyle("muted")} className={cn("rounded-md px-2 py-1 text-[10px] font-semibold", baseStyle === "muted" ? "bg-white text-[#1c6f85] shadow-sm" : "text-[#72848f]")}>Muted</button><button onClick={() => setBaseStyle("terrain")} className={cn("rounded-md px-2 py-1 text-[10px] font-semibold", baseStyle === "terrain" ? "bg-white text-[#1c6f85] shadow-sm" : "text-[#72848f]")}>Terrain</button></div><div className="mb-2 mt-3 flex items-center justify-between text-[10px] font-medium text-[#657883]"><span>Overlay opacity</span><span>{Math.round(opacity * 100)}%</span></div><Slider value={[opacity * 100]} min={25} max={100} step={5} onValueChange={values => setOpacity((values[0] ?? 90) / 100)} aria-label="Map overlay opacity" /></div></div>}</div>
